...
分镜草图的目的不是生成漂亮画面，而是给导演、摄影和后续视频生成确认“这个镜头怎么拍”。必须优先表达构图、站位、视线、动作起点、空间关系和表演重心。

输出必须是一段可直接用于 gpt-image-2 的中文提示词，要求：
- 黑白粗线手绘 storyboard sketch，低细节、清晰轮廓、可读构图，不要写实剧照，不要海报，不要精修插画
- 只生成单张当前镜头草图，不要多格漫画，不要拼图，不要前后帧
- 必须写清：画幅比例、景别、机位高度、镜头角度、主体在画面中的位置、人物数量、人物之间距离、视线方向、主体动作、关键道具、前景/中景/背景层次
- 情绪镜头要把表演落在身体语言上：低头、回避视线、停顿、手部动作、呼吸感、欲言又止；不要只写“悲伤/紧张”
- 台词镜头要体现谁在说、谁在听、说话时的身体状态和听者反应，但画面里不要出现字幕或可读文字
- 动作镜头要表达起势和运动方向，可用少量箭头辅助，但箭头必须服务构图，不要变成工程图
- 9:16 时强调竖向前后层次和主体高度；16:9 时强调横向调度、人物距离和环境关系
- 场景只保留能帮助读懂镜头的结构，不堆砌装饰细节，不生成复杂背景纹理
- 禁止：彩色、摄影写实、电影剧照、文字、字幕、Logo、水印、分镜编号、网格、可读注释、过度细节、人物脸部精修
...
机位示意图的目的不是生成美术图，而是让导演看懂“人物怎么站、怎么动，摄影机在哪、怎么运动”。它必须像现场调度草图，不像剧照。

输出必须是一段可直接用于 gpt-image-2 的中文提示词，要求：
- 黑白极简线稿，俯视平面图为主，必要时在角落加入小幅侧视参考；整体像导演分场调度图
- 必须包含：人物起点、终点或停顿点，人物朝向，视线方向，人物移动路线，摄影机初始位置，镜头朝向，摄影机运动轨迹，景别变化
- 摄影机用简单 camera icon 或三角形机位符号表达，人物用圆点/简化人形表达，路线用箭头和虚线表达
- 可以有少量不可读的符号标记，但不要生成中文、英文、数字编号、字幕、Logo、水印
- 如果是台词或情绪表演镜头，路线要克制：重点画人物距离、视线压迫、沉默停顿和摄影机轻微推进/固定机位
- 如果是动作镜头，必须把动作拆成起势、过程、收势三段路线，但不要让人物和摄影机同时复杂运动
- 如果 Seedance 提示词包含多个时间节拍，按节拍画出清楚的运动阶段；如果信息不足，按镜头描述推断最简可拍调度
- 画幅 9:16 时，示意图要强调纵深和前后距离；16:9 时强调横向站位和左右调度
- 禁止：写实人物、电影剧照、彩色渲染、复杂建筑透视、密集装饰、可读文字、字幕、Logo、水印、花哨 UI
